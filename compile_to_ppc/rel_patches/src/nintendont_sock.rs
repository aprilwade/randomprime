...
decl_so_api!{
    ...
    avetcp_init: fn(n: Never),
    avetcp_term: fn(n: Never),

    dns_set_server: fn(n: Never),
    dns_clear_server: fn(n: Never),
    dns_open_addr: fn(name: *const u8, len: u32) -> i32,
    dns_get_addr: fn(fd: i32, arr: *mut u32) -> i32,
    dns_close: fn(fd: i32) -> i32,

    tcp_create: fn(n: Never),
    tcp_bind: fn(n: Never),
    tcp_listen: fn(n: Never),
    tcp_stat: fn(n: Never),
    tcp_getaddr: fn(n: Never),
    tcp_connect: fn(s: i32, arr: *const u32, port: u16) -> i32,
    tcp_accept: fn(n: Never),
    tcp_send: fn(n: Never),
    tcp_receive: fn(n: Never),
    tcp_abort: fn(n: Never),
    tcp_delete: fn(n: Never),
}
    ...
    pub fn accept(&mut self) -> impl Future<Output = Result<(TcpStream, SOSockAddrIn), Error>>
    {
        ...
        poll_fn(move |cx| {
            match convert_sock_res(unsafe {SOAccept(s as i32, &mut addr) }) {
                Ok(i) => Poll::Ready(Ok((TcpStream(i), addr))),
                Err(Error::EAGAIN) => {
