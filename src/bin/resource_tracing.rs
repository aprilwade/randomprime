...
fn create_nothing(pickup_table: &mut HashMap<usize, PickupData>, base_kind: usize)
{
    // Special case for Nothing
    let mut nothing_bytes = Vec::new();
    {
        let mut nothing_pickup = Reader::new(&pickup_table[&base_kind].bytes)
                                        .read::<Pickup>(()).clone();
    ...
    let mut nothing_deps: HashSet<_> = pickup_table[&base_kind].deps.iter()
